...
# code from here: http://jaredgerschler.blog/2018/02/25/the-zhang-suen-thinning-algorithm-introduction-and-applications/
import cv2
import numpy as np
...
def thinning(img):
    #filter_size = 5
    #tmp_image = cv2.GaussianBlur(img,(filter_size,filter_size), 0)
    retval, orig_thresh = cv2.threshold(img,110,255,cv2.THRESH_BINARY)
