...
def getOrientationField(img, block_size):
    ...
    for i in range(half_block_size, x, block_size):
        for j in range(half_block_size, y, block_size):
            # Orientation field
            sum_Vx = 0
            sum_Vy = 0
            sum_for_coherence = 0
    ...
    #---smoothing orientation field and indication of reference point
    #epsilon and label image init
    epsilon = np.zeros(img.shape)
    label_image = np.zeros(img.shape)
    #
    #filter
    filter_size = 5
    low_pass_filter = cv2.GaussianBlur(img,(filter_size,filter_size), 1)
    for i in range(half_block_size, x, block_size):
        for j in range(half_block_size, y, block_size):
            ...
            for u in range(i-half_filter_size, i+half_filter_size):
                for v in range(j-half_filter_size, j+half_filter_size):
                    ...
                    phi_x2 += low_pass_filter[v][u] * phi_x[j][i] 
                    phi_y2 += low_pass_filter[v][u] * phi_y[j][i]
    
            orientationMatSmooth[j][i] = math.atan2(phi_y2, phi_x2)/2

            #Compute sine component (epsilon)
            epsilon[j][i] = math.sin(orientationMatSmooth[j][i])
